...
class HqcblistmailerTableemailtemplate extends JTable {
    ...
    public function bind($array, $ignore = '') {
		...
		if(($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state','com_hqcblistmailer') && $array['state'] == 1)){
			$array['state'] = 0;
		}
        ...
        if(!JFactory::getUser()->authorise('core.admin', 'com_hqcblistmailer.emailtemplate.'.$array['id'])){
            $actions = JFactory::getACL()->getActions('com_hqcblistmailer','emailtemplate');
            $default_actions = JFactory::getACL()->getAssetRules('com_hqcblistmailer.emailtemplate.'.$array['id'])->getData();
            $array_jaccess = array();
